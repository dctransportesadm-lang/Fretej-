...
export function LoginPage({ onLoginSuccess }: LoginPageProps) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!authWindow) {
        setError('Pop-up bloqueado. Por favor, permita pop-ups para este site.');
        setIsLoading(false);
      }
    } catch (err) {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      // Validate origin (optional but recommended in production)
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="text-center space-y-2">
          <div className="mx-auto bg-blue-600 p-3 rounded-xl w-fit shadow-md mb-2">
            <Truck className="h-8 w-8 text-white" />
          </div>
          <CardTitle className="text-2xl font-bold text-gray-900">Gestão de Fretes</CardTitle>
